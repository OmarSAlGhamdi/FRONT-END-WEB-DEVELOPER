...
// const groupedByUniverse = characters.reduce ((acc, character) => {
//   if (!acc[character.universe]) {
//     acc[character.universe] = [];
//   }
//   acc[character.universe].push(character);
//   return acc;
// }, {});

// const groupedArray = Object.values(groupedByUniverse);
...
let uniqueCharacters = characters.reduce((accumulator, character) => {
  const universeCharacters = characters.filter(
    (c) => c.universe === character.universe,);

if (universeCharacters.length === 1) {
...
let totalPowerLevel = characters.reduce((sum, character) => sum + character.power_level, 0);
let averagePowerLevel = totalPowerLevel / characters.length;
